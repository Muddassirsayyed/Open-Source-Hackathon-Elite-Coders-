# Open Source Hackathon 2026 Project Submission

## Participant Details
- **Full Name**: Muddassir Mushtaque Sayyed
- **GitHub Username**: Muddassirsayyed
- **Team Name**: Elite Coders
- **LinkedIn Profile**: https://www.linkedin.com/in/muddassir-mushtaque-163216361/

## Project Details
- **Project Title**: FixMate AI
- **Project Description**: A modern, full-stack web application designed to connect users with trusted local service professionals instantly. Features live location tracking with a custom SVG radar view, smart proximity recommendation calculations, real-time Socket.io booking notifications, English/Hindi/Marathi translations, and an interactive AI chatbot helper.
- **Tech Stack Used**: Next.js 15, React, TypeScript, Tailwind CSS, Framer Motion, Node.js, Express.js, Socket.io, MongoDB Atlas.
- **GitHub Repository Link**: https://github.com/Muddassirsayyed/Open-Source-Hackathon-Elite-Coders-
- **Live Demo Link**: https://client-nine-nu-66.vercel.app
- **Presentation / Demo Video Link**: https://www.youtube.com/watch?example-video

## Open Source Readiness
- [x] My project is public on GitHub
- [x] My repository has a proper README.md
- [x] I have added setup/installation instructions
- [x] I have added screenshots/demo where possible
- [x] I have added a license file
- [x] My project is original and built/updated during the hackathon period

## Memori Labs Sponsor Task
- [x] I have starred the Memori Labs GitHub repository (https://github.com/MemoriLabs/Memori)
- [x] I have followed Memori Labs on LinkedIn (https://www.linkedin.com/company/memorilabs/)
- [x] I have followed Memori Labs on X (https://x.com/memorilab)
- [x] I have checked Memori Labs social links (https://linktr.ee/memorilabs)

## ID Card Verification
- [x] I have generated my ID card from https://oshack.xyz
- [x] If my ID was not verified, I completed the mandatory verification/giveaway form and tried again
